...
const ChangeDoctor = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await DataProvider.getOne<doctor>('doctor', params.id as string)
                if (data.data) {
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    <Box display="flex" justifyContent="end" mt="20px">
                        <Button type="submit" color="secondary" variant="contained" onClick={() => {
                            let changedDoctor = currentDoctor;
                            if(changedDoctor){
                                changedDoctor.firstName = values.firstName
                                changedDoctor.lastName = values.lastName
                                DataProvider.update<doctor>('doctor', params.id as string, changedDoctor)
